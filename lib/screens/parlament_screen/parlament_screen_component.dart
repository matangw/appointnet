...
class ParlamentScreenComponent extends StatefulWidget {
  ...
  @override
  State<ParlamentScreenComponent> createState() =>
      _ParlamentScreenComponentState();
}
...
class _ParlamentScreenComponentState extends State<ParlamentScreenComponent>
  ...
  Widget build(BuildContext context) {
    ...
    if (needToJumpToCertinEvent && !isLoading) {
      int pageToJump =
          events.indexWhere((event) => event.id == eventToJump?.id);
      print('[!] NEED TO JUMP TO EVENT : ' + pageToJump.toString());
      Future.delayed(Duration(milliseconds: 500)).then((value) =>
          _eventController.animateToPage(pageToJump,
    ...
    return Scaffold(
      ...
      body: isLoading
          ? WidgetUtils().loadingWidget(height * 0.6, width * 0.8)
                ...
                child: Stack(
                  children: [
                    Positioned(
                        ...
                        right: 0,
                        top: 0,
                        child: titleWidget(height * 0.35, width)),
                    Positioned(
                        left: 0,
                        right: 0,
                        bottom: 0,
                        child: bottomContainer(
                            height * 0.7, width, localizations)),
                    Positioned(
                      left: height * 0.03,
                      top: height * 0.05,
                      child: InkWell(
                          child: WidgetUtils()
                              .goBackButton(width, height * 0.05, context)),
                    ),
                  ],
                ),
              ),
                ...
                child: Container(
                  ...
                  child: InkWell(
                    ...
                    child: Card(
                      ...
                      child: Center(
                          child: WidgetUtils().customText(parlament.name,
          ...
          Container(
            ...
            child: events.isEmpty
                ...
                : PageView(
                    ...
                    children: eventContainerList(
                        height * 0.8, width * 0.95, localizations),
                  ),
          ),
          SizedBox(
            height: height * 0.025,
  ...
  List<Widget> eventContainerList(
      double height, double width, MaterialLocalizations localizations) {
    List<Widget> result = [];
    if (events.isEmpty) {
      return [];
  ...
  Widget eventContainer(double height, double width, Event event,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          WidgetUtils().customText(
              'Event at: ' + DateFormat.yMMMd().format(event.date),
              color: Colors.black,
              fontWeight: FontWeight.bold),
          Container(
            height: height * 0.8,
            ...
            child: Column(
              ...
              children: [
                dataRow(width, 'Time', Icons.access_time,
                    localizations.formatTimeOfDay(event.time)),
                dataRow(width, 'Location', Icons.location_on_rounded,
                    event.location),
                dataRow(width, 'Attending', Icons.group, '()()() +5 more',
                    usersComingWidget:
                        eventComingWidget(height * 0.1, width * 0.2, event)),
                ...
                Row(
                  ...
                  children: [
                    parlament.managerId ==
                        ...
                        ? InkWell(
                            onTap: () => showDialog(
                                context: context,
                                builder: (context) =>
                                    deleteEventDialog(height, width, event)),
                            child: CircleAvatar(
                              radius: width * 0.05,
                    ...
                    InkWell(
                      onTap: () => model.isUserAttending(event)
                          ? model.cancelAttend(event)
                          : model.confirmAttend(event),
                      child: Container(
                        height: height * 0.1,
                        width: width * 0.4,
                        decoration: BoxDecoration(
                            color: model.isUserAttending(event)
                                ? MyColors().mainBright
                                : Colors.white,
                            borderRadius: BorderRadius.circular(height * 0.25)),
                        child: Center(
                            child: WidgetUtils().customText(
                                model.isUserAttending(event)
                                    ? 'Coming'
                                    : 'Not coming',
                                color: model.isUserAttending(event)
                                    ? Colors.white
  ...
  Widget eventComingWidget(double height, double width, Event event) {
    List<AppointnetUser> usersComing =
        isLoadingUsers ? [] : model.comingToEventUserList(event);
    return InkWell(
      onTap: () => Navigator.pushNamed(context, ShowEventAttendings.tag,
          arguments: [event, model.parlamentUsers]),
      child: Container(
        height: height,
        width: width,
                    ...
                    child: Stack(
                      children: [
                        Positioned(
                            bottom: height * 0.1,
                            left: width * 0.4,
                            child: usersComing.length > 2
                                ? CircleAvatar(
                                    radius: height * 0.3,
                                    backgroundImage: CachedNetworkImageProvider(
                                        usersComing[2].imageUrl as String),
                                  )
                                : Container()),
                        Positioned(
                            bottom: height * 0.1,
                            left: width * 0.2,
                            child: usersComing.length > 1
                                ? CircleAvatar(
                                    radius: height * 0.3,
                                    backgroundImage: CachedNetworkImageProvider(
                                        usersComing[1].imageUrl as String),
                                  )
                                : Container()),
                        Positioned(
                            bottom: height * 0.1,
                            left: 0,
                            child: usersComing.isNotEmpty
                                ? CircleAvatar(
                                    radius: height * 0.3,
                                    backgroundImage: CachedNetworkImageProvider(
                                        usersComing[0].imageUrl as String),
                                  )
                                : Container()),
                      ],
                    ),
                  ),
                  model.comingToEventUserList(event).length > 3
                      ? WidgetUtils().customText('  +' +
                          model.comingToEventUserList(event).length.toString())
                      : Container()
                ],
              ),
      ),
  ...
  Widget newEventButton(double height, double width) {
    return InkWell(
      onTap: () => Navigator.pushNamed(context, NewEventComponent.tag,
          arguments: parlament),
      child: Container(
        height: height,
        width: width,
        ...
        child: Center(
            child: WidgetUtils().customText('NEW EVENT',
  ...
  AlertDialog deleteEventDialog(double height, double width, event) {
    return AlertDialog(
      title: WidgetUtils().customText(
          'Are you sure you want to delete the event?',
          maxLines: 5),
      actions: [
        InkWell(
          onTap: () => {model.deleteEvent(event), Navigator.pop(context)},
          child: Container(
            height: height * 0.1,
            width: width * 0.4,
            decoration: BoxDecoration(
  ...
  void onFinishedLoading() {
    ...
    Future.delayed(Duration(milliseconds: 100))
        .then((value) => model.setLocalData(events));
  }
  ...
  void finishedAddingUserToParlament(bool success) {
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: WidgetUtils()
            .customText('User added successfully!', color: Colors.white),
  ...
  void deleteEvent(Event event) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: WidgetUtils()
          .customText('Event deleted succsessfuly', color: Colors.white),
      backgroundColor: MyColors().mainBright,
    ));
    setState(() => {});
  }
  ...
  void onLocalDataLoad() {
    setState(() => {isLoading = false, isLoadingUsers = false});
  }
}
