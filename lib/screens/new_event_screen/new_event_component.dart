...
class _NewEventComponentState extends State<NewEventComponent> implements NewEventView{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: loading? WidgetUtils().loadingWidget(height, width)
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              dataRow(height*0.1, width*0.9, Icons.location_on, 'Location', locationField(height*0.06, width*0.4)),
  ...
  Widget confirmButton(double height,double width,MaterialLocalizations localizations){
    return InkWell(
      onTap: ()=>onSubmit(height, width, localizations),
      child: WidgetUtils().bottomButton(
        width,
        height,
  ...
  Widget myAlertDialog(double height,double width,MaterialLocalizations localizations){
    return AlertDialog(
      title: Text(
        'Are you sure you want to add event for '+'parlament name, at '+DateFormat.yMMMd().format(date as DateTime)
        +' - '+localizations.formatTimeOfDay(time as TimeOfDay)+' in '+locationController.text +'?'   ,),
      actions: [
        InkWell(
          onTap: ()=>model.uploadEvent(date as DateTime,time as TimeOfDay,locationController.text),
          child: Container(
  ...
  void onSubmit(double height,double width,MaterialLocalizations localizations){
    String? error = model.submitError(date, time, locationController.text);
    if(error==null){
      showDialog(context: context, builder: (_)=>myAlertDialog(height, width, localizations));
    }
    else
      {
        onError(error);
