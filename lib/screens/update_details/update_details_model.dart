...
class UpdateDetailsModel {
  ...
  Future<void> createAppointnetUser(
      String? userImage, String? name, DateTime? birthDate) async {
    if (userImage == null) {
      ...
      view.onSubmit();
      AppointnetUser user = AppointnetUser(
          name: name,
          phoneNumber: auth.currentUser?.phoneNumber as String,
          birthDate: birthDate);
      String imageUrl =
          await repository.uploadPhoto(File(userImage as String), user);
      user.imageUrl = imageUrl;
      await repository.updateUser(user).then((sucssess) => sucssess
          ? print('[+] USER UPDATED')
          : print('[-] SOMETHING WENT WRONG WITH USER UPDATE'));
