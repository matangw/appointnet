...
class EventRepository {
  ...
  EventRepository({required this.parlamentId}) {
    _eventCollection = FirebaseFirestore.instance
        .collection('parlaments')
        .doc(parlamentId)
        .collection('events');
  }
  ...
  Future<List<Event>> getUpcomingParlamentEvents() async {
    List<Event> result = [];
    QuerySnapshot snap = await _eventCollection
        .where('date',
            isGreaterThan:
  ...
  Future<List<Event>> upcomingUserEvents() async {
    ...
    QuerySnapshot snap = await groupCollecion
        .where('invited', arrayContains: userId)
        .where('date',
            isGreaterThan:
                DateTime.now().subtract(Duration(days: 1)).toString())
        .orderBy('date')
