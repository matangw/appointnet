...
class GeneralUtils {
  String? phoneValidationError(String userInput) {
    if (userInput.length != 10 && userInput.length != 12) {
      return 'Not in the correct length';
    } else if (userInput.startsWith('05') == false &&
        userInput.startsWith('972') == false) {
  ...
  void successSnackBar(String error, BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        backgroundColor: Colors.white,
        content: WidgetUtils().customText(error, color: MyColors().mainColor)));
  }

  Future<void> signOut(BuildContext context) async {
    await FirebaseAuth.instance.signOut();
    Navigator.of(context).pushNamedAndRemoveUntil(
        LoginComponent.tag, (Route<dynamic> route) => false);
  }
